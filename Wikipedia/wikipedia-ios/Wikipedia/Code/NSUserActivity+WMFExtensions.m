...
+ (instancetype)wmf_placesActivityWithURL:(NSURL *)activityURL {
    ...
    // Check for content in the activityURL
    for (NSURLQueryItem *item in components.queryItems) {
        if ([item.name isEqualToString:@"WMFArticleURL"]) {
            NSString *articleURLString = item.value;
            activity.webpageURL = [NSURL URLWithString:articleURLString];
        } else if ([item.name isEqualToString:@"WMFCoordinates"]) {
            if ([self locationFromQueryValueString: item.value]) {
                NSMutableDictionary *updatedUserInfo = activity.userInfo.mutableCopy;
                updatedUserInfo[@"WMFLocation"] = item.value;
...
+ (CLLocation *)locationFromQueryValueString:(NSString *)value {
    NSArray *coordinates = [value componentsSeparatedByString:@","];
    if (coordinates.count == 2) {
        CLLocation *location = [[CLLocation alloc] initWithLatitude:[coordinates[0] doubleValue]
...
- (CLLocation *)wmf_location {
    return [NSUserActivity locationFromQueryValueString:self.userInfo[@"WMFLocation"]];
}
