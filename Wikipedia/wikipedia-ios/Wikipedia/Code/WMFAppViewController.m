...
- (BOOL)processUserActivity:(NSUserActivity *)activity animated:(BOOL)animated completion:(dispatch_block_t)done {
    ...
    switch (type) {
        ...
        case WMFUserActivityTypePlaces: {
            ...
            // Check whether there is content to show on the places tab
            // Since content can be optional it doesn't make sense to add a new activity type
            NSURL *articleURL = activity.wmf_linkURL;
            CLLocation *location = activity.wmf_location;
            
            if (articleURL || location) {
                // For "View on a map" action to succeed, view mode has to be set to map.
                [[self placesViewController] updateViewModeToMap];
            
                // Prefer articleURL over location (since it provides more initial info to the user)
                if (articleURL) {
